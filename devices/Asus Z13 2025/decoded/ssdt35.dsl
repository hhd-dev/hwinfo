 ...
 *
 * Disassembly of ./encoded/ssdt35.dat
 *
 * Original Table Header:
 *     Signature        "SSDT"
 *     Length           0x00001950 (6480)
 *     Revision         0x02
 *     Checksum         0x53
 *     OEM ID           "AMD"
 *     OEM Table ID     "UPEP"
...
DefinitionBlock ("", "SSDT", 2, "AMD", "UPEP", 0x00000001)
{
    ...
    External (_SB_.PCI0.GPP4.CADR, DeviceObj)
    External (_SB_.PCI0.SBRG.EC0_.CSEE, MethodObj)    // 1 Arguments
    External (_SB_.PCI0.SBRG.EC0_.LID_, DeviceObj)
    External (_SB_.PCI0.SBRG.EC0_.ST9A, MethodObj)    // 0 Arguments
    External (_SB_.PCI0.SBRG.EC0_.ST9B, MethodObj)    // 0 Arguments
    ...
    Scope (\_SB)
    {
        Device (PEP)
        {
            Name (_HID, "AMDI000B")  // _HID: Hardware ID
                        ...
                        Case (One)
                        {
                            ...
                            If (CondRefOf (\F5D3))
                            {
                                If ((\F5D3 != 0x04))
                                {
                                    ...
                                    MPEP (Zero, "\\_SB.PCI0.GPPA.ACP", Zero, Zero)
                                }
                            }
                            ...
                            If (CondRefOf (\LOD3))
                            {
                                If ((\LOD3 == One))
                                {
                                    M460 ("    Case1 LOD3 : GPP3 D3 supported\n", Zero, Zero, Zero, Zero, Zero, Zero)
                            ...
                            If (CondRefOf (\SDD3))
                            {
                                If ((\SDD3 == One))
                                {
                                    M460 ("    Case1 SDD3 : GPP4 D3 supported\n", Zero, Zero, Zero, Zero, Zero, Zero)
                            ...
                            If (CondRefOf (\WLD3))
                            {
                                If ((\WLD3 == One))
                                {
                                    M460 ("    Case1 WLD3 : GPP5 D3 supported\n", Zero, Zero, Zero, Zero, Zero, Zero)
                                    MPEP (One, "\\_SB.PCI0.GPP5", Zero, 0x03)
                                }
                            }

                            If ((\NVD3 != 0x55))
                            {
                                MPEP (One, "\\_SB.PCI0.GPP7", Zero, 0x03)
                                MPEP (Zero, "\\_SB.PCI0.GPP7.NVME", Zero, 0x03)
                                MPEP (One, "\\_SB.PCI0.GPP9", Zero, 0x03)
                                MPEP (Zero, "\\_SB.PCI0.GPP9.NVME", Zero, 0x03)
                            }
                            ...
                            If (CondRefOf (\SLX4))
                            {
                                If ((\SLX4 != 0x55))
                                {
                                    M460 ("    Case1 SLX4 : GP10 x4 slot D3 supported\n", Zero, Zero, Zero, Zero, Zero, Zero)
                ...
                ElseIf ((Arg0 == ToUUID ("11e00d56-ce64-47ce-837b-1f898f9aa461") /* Unknown UUID */))
                {
                    Switch (ToInteger (Arg2))
                    {
                        ...
                        Case (0x07)
                        {
                            M000 (0x3E07)
                            M460 ("    Return (0x00)\n", Zero, Zero, Zero, Zero, Zero, Zero)
                            \_SB.PCI0.SBRG.EC0.ST9A ()
                            Return (Zero)
                        }
                        Case (0x08)
                        {
                            M000 (0x3E08)
                            If (CondRefOf (\_SB.PCI0.GPP4.CADR))
                            {
                                M460 ("    Notify (ASL_ASUS_PCIE_CARDREADER_ROOTPORT_SCOPE.CADR, 0x1)\n", Zero, Zero, Zero, Zero, Zero, Zero)
                                Notify (\_SB.PCI0.GPP4.CADR, One) // Device Check
                            }

                            M460 ("    Return (0x00)\n", Zero, Zero, Zero, Zero, Zero, Zero)
                            \_SB.PCI0.SBRG.EC0.ST9B ()
                            Return (Zero)
                        }
                        Default
                        {
