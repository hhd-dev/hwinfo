 ...
 *     Signature        "SSDT"
 *     Length           0x00000F5C (3932)
 *     Revision         0x02
 *     Checksum         0x82
 *     OEM ID           "AMD"
 *     OEM Table ID     "CPMACPV8"
 *     OEM Revision     0x00000001 (1)
 *     Compiler ID      "INTL"
 *     Compiler Version 0x20230331 (539165489)
 */
DefinitionBlock ("", "SSDT", 2, "AMD", "CPMACPV8", 0x00000001)
{
    ...
    Method (M27D, 0, Serialized)
    {
        ...
        Local1 = M249 (Zero, Zero, Zero, 0x01243000)
        M460 ("  ACP_SW_EN   = %d\n", Local1, Zero, Zero, Zero, Zero, Zero)
        Local2 = M249 (Zero, Zero, Zero, 0x01243C00)
        M460 ("  ACP_P1_SW_EN   = %d\n", Local2, Zero, Zero, Zero, Zero, Zero)
        If (((Local1 == One) || (Local2 == One)))
        {
            M460 ("SdW Controller is still running\n", Zero, Zero, Zero, Zero, Zero, Zero)
        }
        ElseIf ((Local0 == Zero))
        {
    ...
    Method (M276, 1, Serialized)
    {
        ...
        If ((APGE == One))
        {
            ...
            ElseIf ((M27B == One))
            {
                M250 (Zero, Zero, Zero, 0x01241024, 0x1F)
                Local0 = 0x64
                Local1 = M249 (Zero, Zero, Zero, 0x01241028)
                While ((Local1 != Zero))
                {
    ...
    Method (M277, 1, Serialized)
    {
        ...
        If ((APGE == One))
        {
            ...
            ElseIf ((M27B == One))
            {
                M250 (Zero, Zero, Zero, 0x01241024, 0x1E)
                Local0 = 0x64
                Local1 = M249 (Zero, Zero, Zero, 0x01241028)
                While ((Local1 != 0xAA))
                {
